...
| 팀원 | 역할 | 담당 업무 |
|------|------|-----------|
| **구○○** (팀장) | 프로젝트 관리, 터치 시스템 설계 | 전체 일정 관리, 터치 하드웨어 설계 |
| **김○○** | 터치 시스템, 통합 | PCB 설계, 터치패드 설계 |
| **류○○** | 터치 시스템 | 회로 제작, 터치 프로그램 개발 | 4인 체스 게임 개발 |
| **조○○** | 기구 설계, 통합 | 외장 디자인, 3D 모델링, 피아노 타일 게임 개발 |
| **전○○** | YOLO 카드 인식 기반 포커 게임 개발, PYQT5 기반 게임 런쳐 개발 |
...
### 하드웨어 관련
- 터치패드 일부 영역에서 압력 감지 정확도 차이 존재
- 특정 조명 조건에서 카드 인식 정확도 감소

### 소프트웨어 관련
- 현재 CPU 기반으로 객체인식에 소요되는 시간이 길어 JetPack 활용 필요
- 매우 빠른 게임 진행 시 UI 지연 가능
- 1920*1080에 고정된 UI 사용 -> 적응형 UI로 개선 필요
