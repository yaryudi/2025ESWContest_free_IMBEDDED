...
class CalibrationThread(QThread):
    ...
    def __init__(self, ser, offset):
        ...
        self.last_touch_coords = None  # 마지막 터치 좌표
        self.double_click_threshold = 0.5  # 0.5초 내 같은 위치 터치 시 더블클릭으로 간주
        self.double_click_distance = 5  # 같은 위치로 간주할 거리 (픽셀)
                ...
                if frame is not None:
                    ...
                    if peak:
                        ...
                        if v >= self.TOUCH_THRESHOLD:
                            ...
                            # 더블클릭 방지 로직
                            if self.is_double_click(r, c, current_time):
                                print(f"더블클릭 감지됨 - 무시: ({r}, {c})")
                                continue
                            ...
                            if current_time - self.last_touch_time >= self.cool_down_time:
                                ...
                                self.last_touch_coords = (r, c, current_time)
    ...
    def is_double_click(self, r, c, current_time):
        """더블클릭인지 확인"""
        if self.last_touch_coords is None:
            return False
        
        last_r, last_c, last_time = self.last_touch_coords
        
        # 시간 체크 (0.5초 내)
        if current_time - last_time > self.double_click_threshold:
            return False
                ...
                if frame is not None:
                    ...
                    if peak:
                        ...
                        if v >= self.TOUCH_THRESHOLD:
                            ...
                            # 더블클릭 방지 로직
                            if self.is_double_click(r, c, current_time):
                                print(f"마우스 제어 - 더블클릭 감지됨 - 무시: ({r}, {c})")
                                continue
...
class CalibrationGUI(QMainWindow):
    ...
    def start_next_step(self):
        ...
        self.touch_detected_in_step = False  # 터치 감지 상태도 초기화
    ...
    def on_touch_detected(self, coords):
        ...
        if self.current_step >= 1 and self.current_step <= 4:
            # 더블클릭 방지 - 같은 위치에서 연속 터치 방지
            if self.last_touch_coords and self.is_same_location(r, c):
                print(f"캘리브레이션 - 같은 위치 연속 터치 무시: ({r}, {c})")
                return
    ...
    def is_same_location(self, r, c):
        ...
        distance = ((r - last_r) ** 2 + (c - last_c) ** 2) ** 0.5
        return distance <= 5  # 5픽셀 이내면 같은 위치로 간주
